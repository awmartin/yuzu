...
module Wren::Generators
  class PaginateGenerator < Generator
    ...
    def generate!(website_file)
      ...
      return if website_file.is_a?(Wren::Core::PaginatedWebsiteFile) or \
  ...
  class CatalogPaginator
    ...
    def catalogs_from_args(catalog_args)
      catalog_args.collect {|args| Wren::Filters.catalog_for(@website_file, args)}
    end

    # Given a Catalog instance, check to see if it requires pagination and generate the appropriate
    # files for the given content.
    # 
    # @param [Catalog] catalog The Wren::Generator.Catalog object that represents the contents to
    ...
    def generate_page!(page, catalog)
      ...
      paginated_file = Wren::Core::PaginatedWebsiteFile.new(@website_file, new_raw_contents, page)
