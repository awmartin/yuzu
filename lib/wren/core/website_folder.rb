...
module Wren::Core
  class WebsiteFolder < WebsiteBase
    ...
    def breadcrumb
      Wren::Renderers::BreadcrumbRenderer.new.render(self)
    end

    def currentpath
      Wren::Filters::CurrentpathFilter.new.value(self)
    end

    def name
      @path.rootname.titlecase
      #Wren::Filters::PostTitleFilter.new.extract_title_from_filename(@path)
    end
